"""
Code for reproducing the results of TABLE 2.
"""
...
with open("output/esn_mnist_2.txt", "w") as f:
    for radius in radius_ls:
        f.write(f"radius: {radius}\n")
        print(f"radius: {radius}")
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        esn_model = ESNClassifier(28 * 28, 500, 10, radius, 0.1, 0.1, 28, device).to(device)
        ...
        for i in range(num_epochs):
            ...
            for inputs, targets in tqdm.tqdm(train_loader):
                ...
                # repeat the input for 28 times
                inputs = inputs.unsqueeze(1).repeat(1, 28, 1)
        ...
        # save the model
        torch.save(esn_model.state_dict(), f"output/esn_mnist_2_{radius}.pth")
